...
final class DriveSession {
    ...
    // MARK: Drive mode (0 = normal, 1 = offroad, 2 = racing, 3 = auto-generated)
    var driveModeRaw: Int = 0
    ...
    // MARK: - Connecting drive initializer

    /// Creates an auto-generated placeholder drive connecting the end of one session to the
    /// start of another. Route coordinates come from a MapKit directions response.
    /// No sensor data is stored; this drive does not receive a smoothness score.
    init(connectingFrom startCoord: CLLocationCoordinate2D,
         to endCoord: CLLocationCoordinate2D,
         departureDate: Date,
         routeCoordinates: [CLLocationCoordinate2D],
         estimatedDurationSeconds: Double,
         estimatedDistanceM: Double) {
        let avgSpeed = estimatedDistanceM / max(1, estimatedDurationSeconds)
        startDate           = departureDate
        durationSeconds     = estimatedDurationSeconds
        stoppingTimeSeconds = 0
        stopCount           = 0
        totalDistanceM      = estimatedDistanceM
        maxSpeedMps         = avgSpeed
        avgSpeedMps         = avgSpeed
        avgMovingSpeedMps   = avgSpeed
        routeLatitudes      = routeCoordinates.map { $0.latitude }
        routeLongitudes     = routeCoordinates.map { $0.longitude }
        routeSpeeds         = Array(repeating: avgSpeed, count: routeCoordinates.count)
        routeAltitudes      = Array(repeating: 0.0, count: routeCoordinates.count)
        driveModeRaw        = DriveMode.autoGenerated.rawValue
        smoothnessScore     = 0
        // All acceleration / jerk / event fields remain at default (0 / [])
        peakForward = 0; peakBraking = 0; avgLongitudinalAbs = 0; rmsForward = 0
        hardAccelCount = 0; hardBrakingCount = 0
        peakJerkForward = 0; peakJerkBraking = 0; avgJerkLongitudinalAbs = 0
        peakRight = 0; peakLeft = 0; avgLateralAbs = 0; rmsLateral = 0
    ...
    var driveMode: DriveMode {
        get { DriveMode(rawValue: driveModeRaw) ?? .normal }
