...
struct DriveSessionView: View {
    ...
    @State private var showingNameStartSheet = false
    @State private var showingNameEndSheet = false
    @State private var renamingStart = false
    @State private var renamingEnd = false
    @State private var pendingPlaceName = ""
    private var stats: SessionStats { SessionStats(restoringFrom: session) }

    private var startCoordinate: CLLocationCoordinate2D? {
        guard let lat = session.routeLatitudes.first,
              let lon = session.routeLongitudes.first else { return nil }
        return CLLocationCoordinate2D(latitude: lat, longitude: lon)
    }

    private var endCoordinate: CLLocationCoordinate2D? {
        guard let lat = session.routeLatitudes.last,
              let lon = session.routeLongitudes.last else { return nil }
        return CLLocationCoordinate2D(latitude: lat, longitude: lon)
    }
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                if session.driveMode == .autoGenerated {
                    ...
                    Menu {
                        Picker("Drive Mode", selection: Binding(
                            ...
                            ForEach(DriveMode.allCases.filter { $0 != .autoGenerated }, id: \.self) { mode in
                                Label(mode.label, systemImage: mode.sfSymbol).tag(mode)
                            }
                        }
                        if session.routeLatitudes.count >= 2 {
                            Divider()
                            Button {
                                pendingPlaceName = session.startPlaceName ?? ""
                                renamingStart = true
                            } label: {
                                Label("Rename Start Place…", systemImage: "pencil")
                            }
                            Button {
                                pendingPlaceName = session.endPlaceName ?? ""
                                renamingEnd = true
                            } label: {
                                Label("Rename End Place…", systemImage: "pencil")
                            }
                            Divider()
                            Button {
                                showingNameStartSheet = true
                            } label: {
                                Label("Save Start as Named Location", systemImage: "mappin.circle")
                            }
                            Button {
                                showingNameEndSheet = true
                            } label: {
                                Label("Save End as Named Location", systemImage: "mappin.circle.fill")
                            }
                        }
        ...
        .sheet(isPresented: $showingNameStartSheet) {
            NamedLocationFormView(existing: nil, initialCoordinate: startCoordinate)
        }
        .sheet(isPresented: $showingNameEndSheet) {
            NamedLocationFormView(existing: nil, initialCoordinate: endCoordinate)
        }
        .alert("Rename Start Place", isPresented: $renamingStart) {
            TextField("Place name", text: $pendingPlaceName)
            Button("Save") {
                let v = pendingPlaceName.trimmingCharacters(in: .whitespaces)
                session.startPlaceName = v.isEmpty ? nil : v
                try? modelContext.save()
            }
            Button("Cancel", role: .cancel) {}
        }
        .alert("Rename End Place", isPresented: $renamingEnd) {
            TextField("Place name", text: $pendingPlaceName)
            Button("Save") {
                let v = pendingPlaceName.trimmingCharacters(in: .whitespaces)
                session.endPlaceName = v.isEmpty ? nil : v
                try? modelContext.save()
            }
            Button("Cancel", role: .cancel) {}
        }
